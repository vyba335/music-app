        ...
        <div className="absolute bottom-0 w-full p-4">
            <div className="space-y-3">
                ...
                <div className="flex space-x-2 mt-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="w-16 h-16 bg-gray-600 rounded flex-shrink-0"></div>
                    ))}
                </div>
            </div>
        </div>
    ...
    <div className="bg-[#272932] rounded-lg shadow-lg p-6 animate-pulse">
        ...
        <div className="space-y-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="h-12 bg-gray-700 rounded border border-gray-600"></div>
                ))}
            </div>
...
export const LoadingSpinner = ({ size = "md", color = "text-[#95c623]" }: { size?: "sm" | "md" | "lg", color?: string }) => {
    const sizeClasses = {
        sm: "w-4 h-4",
        md: "w-6 h-6", 
        lg: "w-8 h-8"
